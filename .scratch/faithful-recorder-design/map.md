...
- [Define verification and acceptance contract](issues/09-define-verification-and-acceptance-contract.md) — two fully automated layers: byte-level replay tests on the captured fixture (bidirectional byte fidelity, admission order, streaming-first-bytes, auxiliary traffic excluded, normal-stop semantics, synthetic keep-alive and empty-entity scenarios; delivery-chunk boundaries deliberately not asserted) plus live acceptance driving both entrypoints from the command line (claude -p, and the extension-shipped binary with its launcher env — no VS Code UI), pass = tests green plus both live scenarios recorded.
